...
export default function BiuroDodajZlecenieScreen() {
  ...
  const zapiszZlecenieDoBazy = async (docelowyStatus: string) => {
    ...
    try {
      ...
      const { data: profilBiura, error: errProfil } = await supabase
        .from("pracownicy")
        .select("id, id_firmy")
        .eq("rola", "biuro")
        .limit(1)
      ...
      if (!profilBiura) {
        Alert.alert("Błąd", "Nie znaleziono profilu biura.");
        return;
      }
      ...
      const { data: noweZlecenie, error: errZlecenie } = await supabase
        ...
        .insert([
          {
            numer_zd: numerZD,
            id_firmy: profilBiura.id_firmy,
            id_zlecajacego: profilBiura.id,
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Skanuj i przydzielaj ZD (Biuro)</Text>
        </View>
