...
export default function KrojowniaScreen() {
  ...
  const fetchZlecenia = async (cicheOdswiezanie = false) => {
    ...
    try {
      const { data: zleceniaData, error: errZlecenia } = await supabase
        .from("zlecenia")
        .select("*")
        .in("status", [
          "oczekuje_krojownia",
  ...
  // ZMIANA 1: Całkowicie usunięta funkcja toggleSzycie - Krojcza już nie klika pozycji.

  const handleZatwierdzQC = async () => {
    if (!qcZlecenie) return;
    setLoading(true); // WŁĄCZAMY KÓŁKO
    try {
      await supabase
        .from("zlecenia")
      ...
      // >>> TUTAJ NAPRAWA: ZDEJMUJEMY KÓŁKO PRZED CICHYM ODŚWIEŻENIEM <<<
      setLoading(false);
  ...
  const handleOdrzucQC = async () => {
    ...
    try {
      ...
      if (Platform.OS === "web")
        window.alert(`Odrzucono! ZD ${qcZlecenie.numer_zd} wraca do szwalni.`);
      else
        Alert.alert(
          "Odrzucono!",
  ...
  const handleSkanujZlecenie = async (zeskanowanyKod?: string) => {
    ...
    try {
      ...
      if (aktualnyStatus === "oczekuje_krojownia") {
        ...
        const { data: otwartyLog } = await supabase
          .from("logi_pracy")
          .select("id")
          .eq("id_zlecenia", zlecenie.id)
          .eq("id_pracownika", idPracownika)
          .eq("etap_pracy", "krojenie")
          .is("czas_stop", null)
          .single();
        if (otwartyLog)
          await supabase
      ...
      >
        ...
        {isExpanded && (
          <View style={styles.expandedContent}>
            {/* ZMIANA 2: Nowy tekst informacyjny */}
            ...
            {produktyZlecenia.length === 0 ? (
              ...
              produktyZlecenia.map((prod) => {
                ...
                return (
                  // ZMIANA 3: View zamiast TouchableOpacity (Brak możliwości zmiany!)
                  <View
                    key={prod.id}
                    style={[
                      styles.pozycjaRow,
            ...
            {!isRejecting ? (
              <>
                ...
                <View style={styles.qcActionButtonsRow}>
                  ...
                  >
                    <Text style={[styles.qcDecisionBtnText, { color: "#fff" }]}>
                      ✅ ZATWIERDŹ (GOTOWE)
                    </Text>
                  </TouchableOpacity>
                </View>
...
const styles = StyleSheet.create({
  ...
  checkboxText: { color: "#fff", fontWeight: "bold", fontSize: 14 }, // Lekko zmniejszyłem font, żeby zmieściło ikonki emoji
