...
export default function UbieralniaScreen() {
  ...
  const fetchZlecenia = async (cicheOdswiezanie = false) => {
    ...
    try {
      const { data: zleceniaData } = await supabase
        .from("zlecenia")
        .select("*")
        .in("status", ["oczekuje_ubieralnia", "ubieranie_w_trakcie"])
        .order("utworzono", { ascending: true });

      if (zleceniaData) {
        setZleceniaGotowe(zleceniaData.filter((z) => z.status === "oczekuje_ubieralnia"));

        const { data: logiData } = await supabase
          .from("logi_pracy")
          .select("*")
          .eq("id_pracownika", idPracownika)
          .eq("etap_pracy", "ubieranie")
        ...
        const wszystkieSzyte = zleceniaData.filter((z) => z.status === "ubieranie_w_trakcie");
  ...
  const filtrujUwagiProdukcji = (instrukcje: string) => {
    if (!instrukcje) return "";
    const indexProdukcja = instrukcje.toUpperCase().indexOf("PRODUKCJA");
    
    if (indexProdukcja !== -1) {
      let tekstPo = instrukcje.substring(indexProdukcja + 9).trim();
      if (tekstPo.startsWith(":")) tekstPo = tekstPo.substring(1).trim();
      return tekstPo;
    }
    
    if (instrukcje.toUpperCase().includes("SZWALNIA")) {
      return ""; 
    }
  ...
  const handleStartPozycji = async (zlecenieId: string, pozycjaId: string, idFirmy: string) => {
    const pracownikIdStr = Array.isArray(idPracownika) ? idPracownika[0] : idPracownika;
    ...
    if (isOnline) {
      await supabase
        ...
        .match({ id_pracownika: pracownikIdStr, etap_pracy: "ubieranie" })
        .is("czas_stop", null);

      const { error } = await supabase.from("logi_pracy").insert([
        { id_zlecenia: zlecenieId, id_pracownika: pracownikIdStr, id_firmy: idFirmy, etap_pracy: "ubieranie", id_pozycji: pozycjaId },
      ]).select();

      if (error) {
        Alert.alert("🚨 BŁĄD ZAPISU W BAZIE", "Supabase odrzucił log czasu!\nPowód: " + error.message);
        fetchZlecenia(true); 
        return; 
      }
      ...
      await SyncManager.dodajDoKolejki("logi_pracy", "UPDATE", 
        ...
        { id_pracownika: pracownikIdStr, etap_pracy: "ubieranie", czas_stop: null }
      );
  ...
  const handleZatwierdzPauze = async (powod: string) => {
    ...
    if (isOnline) {
      await supabase
        ...
        .match({ id_pracownika: idPracownika, etap_pracy: "ubieranie" })
        .is("czas_stop", null);
      fetchZlecenia(true);
    } else {
      await SyncManager.dodajDoKolejki("logi_pracy", "UPDATE", 
        { czas_stop: new Date().toISOString(), uwagi: `PAUZA: ${powod}` }, 
        { id_pracownika: idPracownika, etap_pracy: "ubieranie", czas_stop: null }
      );
    }
  ...
  const handleWznow = async (zlecenieId: string, idFirmy: string) => {
    ...
    if (isOnline) {
      const { error } = await supabase.from("logi_pracy").insert([
        { id_zlecenia: zlecenieId, id_pracownika: idPracownika, id_firmy: idFirmy, etap_pracy: "ubieranie", id_pozycji: pozycjaId },
      ]);
  ...
  const handleZakonczWozekRaw = async (zlecenie: any) => {
    const wykonajZakonczenie = async () => {
      ...
      try {
        if (isOnline) {
          ...
          await supabase.from("zlecenia").update({ status: "oczekuje_pikowanie" }).eq("id", zlecenie.id);
          await supabase.from("historia_statusow").insert([
            { id_zlecenia: zlecenie.id, id_pracownika: idPracownika, id_firmy: zlecenie.id_firmy, stary_status: "ubieranie_w_trakcie", nowy_status: "oczekuje_pikowanie" },
          ]);

          if (Platform.OS === "web") window.alert(`Super! ZD ${zlecenie.numer_zd} jedzie do Pikowaczy.`);
          else Alert.alert("Super!", `Wózek jedzie do Pikowaczy!`);
        } else {
          await SyncManager.dodajDoKolejki("logi_pracy", "UPDATE", 
            { czas_stop: new Date().toISOString() }, 
            { id_pracownika: idPracownika, etap_pracy: "ubieranie", czas_stop: null }
          );
          await SyncManager.dodajDoKolejki("zlecenia", "UPDATE", { status: "oczekuje_pikowanie" }, zlecenie.id);
          await SyncManager.dodajDoKolejki("historia_statusow", "INSERT", { id_zlecenia: zlecenie.id, id_pracownika: idPracownika, id_firmy: zlecenie.id_firmy, stary_status: "ubieranie_w_trakcie", nowy_status: "oczekuje_pikowanie" });
    ...
    if (Platform.OS === "web") {
      if (window.confirm(`✅ Zakończyć materac z ${zlecenie.numer_zd} i wysłać do pikowania?`)) wykonajZakonczenie();
    } else {
      Alert.alert("✅ Zakończyć?", `Gotowe do pikowania: ${zlecenie.numer_zd}?`, [
  ...
  const handleSkanujZlecenie = async (zeskanowanyKod?: string) => {
    ...
    try {
      ...
      if (aktualnyStatus === "oczekuje_ubieralnia") {
        if (isOnline) {
          await supabase.from("zlecenia").update({ status: "ubieranie_w_trakcie" }).eq("id", zlecenie.id);
          await supabase.from("historia_statusow").insert([
            { id_zlecenia: zlecenie.id, id_pracownika: idPracownika, id_firmy: zlecenie.id_firmy, stary_status: aktualnyStatus, nowy_status: "ubieranie_w_trakcie" },
          ]);
        } else {
          await SyncManager.dodajDoKolejki("zlecenia", "UPDATE", { status: "ubieranie_w_trakcie" }, zlecenie.id);
          await SyncManager.dodajDoKolejki("historia_statusow", "INSERT", { id_zlecenia: zlecenie.id, id_pracownika: idPracownika, id_firmy: zlecenie.id_firmy, stary_status: aktualnyStatus, nowy_status: "ubieranie_w_trakcie" });
          
          setZleceniaGotowe(prev => prev.filter(z => z.id !== zlecenie.id));
          setZleceniaWTrakcie(prev => [...prev, { ...zlecenie, status: "ubieranie_w_trakcie" }]);
        }
        setExpandedId(zlecenie.id);
      } else if (aktualnyStatus === "ubieranie_w_trakcie") {
        setExpandedId(zlecenie.id);
      } else {
        Alert.alert("Informacja", `Zlecenie ma status: ${aktualnyStatus}. Nie można go ubrać.`);
      }
  ...
  const renderZlecenieAktywne = (z: any) => {
    ...
    return (
      <View key={z.id} style={[styles.listItem, { borderLeftColor: "#10b981" }]}>
        <TouchableOpacity style={styles.listHeader} onPress={() => setExpandedId(isExpanded ? null : z.id)}>
          <View style={{ flex: 1 }}>
            ...
            {idSzytejTerazPozycji ? (
              <Text style={[styles.listTextSub, { color: "#10b981", fontWeight: "bold" }]}>Montujesz - Czas leci</Text>
            ) : (
              <Text style={[styles.listTextSub, { color: "#f59e0b", fontWeight: "bold" }]}>Oczekuje - Wybierz produkt!</Text>
            )}
          </View>
          {idSzytejTerazPozycji && (
        ...
        {isExpanded && (
          <View style={styles.expandedContent}>
            {produktyZlecenia.map((prod) => {
              ...
              const uwagiProdukcji = filtrujUwagiProdukcji(prod.instrukcje);
              ...
              return (
                <View key={prod.id} style={[styles.pozycjaRow, czySzyteTeraz && styles.pozycjaAktywna]}>
                  ...
                  {uwagiProdukcji.length > 0 ? (
                    <View style={styles.instrukcjeBox}>
                      <Text style={styles.instrukcjeLabel}>🛠️ UWAGI DLA UBIERALNI:</Text>
                      <Text style={styles.instrukcjeText}>{uwagiProdukcji}</Text>
                    </View>
                  ) : null}
                  ...
                  <View style={{ alignItems: "flex-start", marginTop: 10 }}>
                    {czySzyteTeraz ? (
                      <View style={styles.szyteTerazBadge}>
                        <Text style={styles.szyteTerazText}>🟢 MONTUJESZ TO TERAZ</Text>
                      </View>
                    ) : (
                      <TouchableOpacity style={styles.startProductBtn} onPress={() => handleStartPozycji(z.id, prod.id, z.id_firmy)}>
                        <Text style={styles.startProductText}>▶️ ZACZNIJ MONTOWAĆ</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
            ...
            <TouchableOpacity style={styles.endCartButton} onPress={() => handleZakonczWozekRaw(z)}>
              <Text style={styles.endCartButtonText}>✅ GOTOWE -> DO PIKOWANIA</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
  ...
  const renderZlecenieOczekujace = (z: any) => (
    <View key={z.id} style={[styles.listItem, { borderLeftColor: "#8b5cf6", opacity: 0.7 }]}>
      <View style={styles.listHeader}>
        <View style={{ flex: 1 }}>
          <Text style={styles.listTextBold}>{z.numer_zd}</Text>
          <Text style={styles.listTextSub}>Oczekuje na montaż</Text>
        </View>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#f0fdf4" }}>
      ...
      <KeyboardAvoidingView 
        style={[styles.container, !isOnline && { paddingTop: 10 }]} // Zmniejszony margines jak jest pasek
      ...
      >
        <Modal visible={isPauseModalVisible} transparent={true} animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <TouchableOpacity style={styles.modalOptionBtn} onPress={() => handleZatwierdzPauze("Brak pianki/wkładu")}>
                <Text style={styles.modalOptionText}>📦 Brak pianki / wkładu</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.modalOptionBtn} onPress={() => handleZatwierdzPauze("Awaria sprzętu")}>
                <Text style={styles.modalOptionText}>🛠️ Awaria sprzętu</Text>
              </TouchableOpacity>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>Panel Ubieralni</Text>
          <Text style={styles.subtitle}>Montaż: {nazwaPracownika}</Text>
        </View>

        <View style={styles.listWrapper}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20, alignItems: "center" }}
            refreshControl={<RefreshControl refreshing={loading} onRefresh={() => fetchZlecenia(false)} colors={["#3b82f6"]} />}
          >
            <View style={styles.card}>
              <Text style={styles.label}>PODEJMIJ WÓZEK DO MONTAŻU:</Text>
            ...
            {!loading && (
              <View style={{ width: "100%", maxWidth: 500 }}>
                {zleceniaWTrakcie.length > 0 && (
                  <>
                    <Text style={styles.sectionHeader}>🟢 TWOJE WÓZKI W TRAKCIE MONTAŻU ({zleceniaWTrakcie.length}):</Text>
                ...
                <Text style={[styles.sectionHeader, { marginTop: 25 }]}>📋 CZEKAJĄCE OD KIEROWNIKA ({zleceniaGotowe.length}):</Text>
                {zleceniaGotowe.length === 0 && <Text style={styles.emptyText}>Brak wózków do montażu.</Text>}
...
// Zmodyfikowane lekko kolory na chłodny niebieski (motyw Ubieralni)
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: "#f0fdf4", paddingTop: 50 },
  ...
  input: { backgroundColor: "#f0fdf4", borderWidth: 2, borderColor: "#bbf7d0", borderRadius: 12, padding: 15, fontSize: 18, marginBottom: 15 },
  cameraButton: { backgroundColor: "#3b82f6", padding: 18, borderRadius: 12, alignItems: "center" },
  ...
  pauseButton: { backgroundColor: "#fef3c7", paddingVertical: 10, paddingHorizontal: 15, borderRadius: 8, marginRight: 10, borderWidth: 1, borderColor: "#f59e0b" },
  pauseButtonText: { color: "#d97706", fontWeight: "bold", fontSize: 13 },
  resumeButton: { backgroundColor: "#3b82f6", paddingVertical: 10, paddingHorizontal: 20, borderRadius: 12 },
  ...
  pozycjaAktywna: { borderColor: "#3b82f6", backgroundColor: "#eff6ff" },
  ...
  pozycjaDuzaIlosc: { fontSize: 26, fontWeight: "900", color: "#3b82f6", backgroundColor: "#eff6ff", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, overflow: "hidden" },
  instrukcjeBox: { backgroundColor: "#e0f2fe", padding: 15, borderRadius: 8, marginBottom: 10, borderWidth: 2, borderColor: "#bae6fd" },
  instrukcjeLabel: { fontSize: 13, fontWeight: "bold", color: "#0369a1", marginBottom: 5 },
  ...
  szyteTerazBadge: { backgroundColor: "#3b82f6", paddingVertical: 12, paddingHorizontal: 20, borderRadius: 8 },
  ...
  footerContainer: { padding: 20, backgroundColor: "#f0fdf4", borderTopWidth: 1, borderTopColor: "#bbf7d0", alignItems: "center" },
