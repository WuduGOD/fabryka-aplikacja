...
export default function UbieralniaScreen() {
  ...
  // --- FILTR UWAG DLA PRODUKCJI (UBIERALNI) ---
  const filtrujUwagiProdukcji = (instrukcje: string) => {
    ...
    if (indexProdukcja !== -1) {
      // Odcinamy słowo "PRODUKCJA" i zwracamy wszystko co jest po nim
      // Dodajemy +9 żeby pominąć samo słowo P-R-O-D-U-K-C-J-A (i ew. dwukropek)
      let tekstPo = instrukcje.substring(indexProdukcja + 9).trim();
      if (tekstPo.startsWith(":")) tekstPo = tekstPo.substring(1).trim();
      return tekstPo;
    }
    
    // Jeśli nie ma słowa PRODUKCJA, ale jest SZWALNIA - ukrywamy, bo to nie dla nich
    if (instrukcje.toUpperCase().includes("SZWALNIA")) {
      return ""; 
    }
    
    // Jeśli nie ma żadnych z tych słów - pokazujemy całość
    return instrukcje;
  };

const handleStartPozycji = async (zlecenieId: string, pozycjaId: string, idFirmy: string) => {
    // Upewnijmy się, że ID jest czystym tekstem (bezpiecznik)
    const pracownikIdStr = Array.isArray(idPracownika) ? idPracownika[0] : idPracownika;

    // 1. OPTYMISTYCZNY UPDATE UI (zapalamy na zielono natychmiast)
    ...
    // 2. ZAMKNIĘCIE STARYCH ZADAŃ W BAZIE
    await supabase
      .from("logi_pracy")
      .update({ czas_stop: new Date().toISOString() })
      .eq("id_pracownika", pracownikIdStr)
      .eq("etap_pracy", "ubieranie")
      .is("czas_stop", null);

    // 3. WYSŁANIE NOWEGO ZADANIA DO BAZY (z dopiskiem .select() wymuszającym odpowiedź)
    ...
    // 4. OBSŁUGA ODRZUCENIA PRZEZ BAZĘ
    if (error) {
      Alert.alert("🚨 BŁĄD ZAPISU W BAZIE", "Supabase odrzucił log czasu!\nPowód: " + error.message);
      fetchZlecenia(true); // Przywracamy szary przycisk, skoro baza odrzuciła
      return; 
    }

    // 5. Zabezpieczenie przed wyprzedzaniem bazy danych (opóźniamy pobranie o pół sekundy)
    setTimeout(() => {
      fetchZlecenia(true);
    }, 500);
  };
            ...
            {produktyZlecenia.map((prod) => {
              ...
              return (
                <View key={prod.id} style={[styles.pozycjaRow, czySzyteTeraz && styles.pozycjaAktywna]}>
                  ...
                  {/* Użytkownik widzi tylko uwagi dotyczące PRODUKCJI */}
